...
class CollectorScanHistoryScreen extends StatefulWidget {
  const CollectorScanHistoryScreen({super.key});

  @override
  State<CollectorScanHistoryScreen> createState() =>
      _CollectorScanHistoryScreenState();
}

class _CollectorScanHistoryScreenState
    extends State<CollectorScanHistoryScreen> {
  ...
  Future<void> _loadScanHistory() async {
    ...
    try {
      if (currentCollectorId == null) {
        setState(() {
          _errorMessage = "No collector logged in";
          _isLoading = false;
        });
        return;
      }

      print('Loading scan history for collector: $currentCollectorId');

      // Get scans from Firestore directly
      ...
      for (var doc in scansSnapshot.docs) {
        ...
        print(
            'Scan record: ${data['residentName'] ?? 'Unknown Name'}, ID=${doc.id}, timestamp=${data['timestamp']}');
      ...
      print('Scan history loaded successfully: ${_scanHistory.length} records');
    } catch (e) {
      print('Error in _loadScanHistory: $e');
      setState(() {
      ...
      child: Column(
        ...
        children: [
          ...
          const Padding(
            ...
            child: Text(
              'Start scanning QR codes to record resident waste collections.',
              style: TextStyle(
                fontSize: 14,
                color: Colors.grey,
              ),
  ...
  Widget _buildScanCard(Map<String, dynamic> scan) {
    final String residentName = scan['residentName'] ?? 'Unknown Resident';
    ...
    final String barangay = scan['barangay'] ?? 'Unknown';
    final String purok = scan['purok'] ?? 'Unknown';
    ...
    return Card(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Resident name with larger text
          Padding(
            ...
            child: Row(
              children: [
                const CircleAvatar(
                  radius: 20,
                  backgroundColor: Color.fromARGB(20, 3, 144, 123),
                  child: Icon(
